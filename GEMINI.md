# Project Context: Pain Metaphor Comprehension (MA Thesis)
...
- **Providers Configured:** OpenAI, Google GenAI, Anthropic, Replicate (Llama), HuggingFace Endpoint.

### Repository Structure
- `evaluate.py`: The unified CLI tool for running evaluations. Takes `--provider`, `--model`, `--mode` (elenchus or maieutic), `--input`, `--output`, `--temperature`, and `--max_tokens`.
- `.env.example`: Template for required API keys.
- `MA_scriptie/`: Original thesis workspace archive (data, docs, legacy scripts, thesis documents).
  - `data/`: Contains the datasets (`.json` files with `lure` and `utterance` keys) and historical output `.csv` evaluation files.
  - `code/`: Contains the original `evaluate_model.py` (predecessor to root `evaluate.py`).
  - `docs/`: Contains `thesis_summary.md`.
...
2. **LangChain Usage:** For pipelines, stick to `RunnablePassthrough`, `ChatPromptTemplate`, and `StrOutputParser` as established in `evaluate.py`.
